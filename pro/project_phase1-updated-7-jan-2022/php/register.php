...
function isValidUserName($pdo, $user_name)
{
    ...
    $sql = "SELECT COUNT(user_name) AS counter  FROM `users` \n"
...
<form method="post" action="./register.php">
    <fieldset>
     ...
     <label for="mobile">Mobile Number:</label>
      ...
      <input type='text' name='mobile' placeholder='Enter 9 digits mobile number'  pattern="^\\d{9}$" required>
     </div>
...
function register_account($pdo)
{
    ...
    if ($_SESSION['user_type'] == "owner") {
        $result_string = "Dear owner " . $_SESSION['name'] . " your registration is done. and your owner_id is '" .
...
<body>
    ...
    <main>
        ...
        if (isset($_SESSION)  && isset($_SESSION['logged_in'])   && $_SESSION['logged_in'] == "true") {
            echo "<h2 class='welcome'>Welcome " . $_SESSION['user_name'] . "</h2>";
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['user_type']) && !empty($_POST['user_type'])) {
