...
function getTimeTableOfFlat()
{
    ...
    if ($id == PHP_INT_MAX) { // this to handle the invalid url changing
        ...
        $sql_flat = "SELECT * FROM `flat` WHERE  manager_status = 'approved' and id NOT IN ( SELECT flat_id FROM customer_rent_flat WHERE rent_end >= CURRENT_TIMESTAMP )
        order by id ";
    ...
    } else  if ($id != PHP_INT_MAX) { // this means no availabile flats in the database


        $sql_flat = "SELECT * FROM `flat` WHERE  manager_status = 'approved' and id =" . $id  . "";
        $result_flat = $pdo->query($sql_flat);
    ...
    while ($row = $result->fetch()) {
        ...
        if (/*$row["is_taken"] == "1" ||*/(date('w') == $arr["" . $row["day"] . ""] &&   strtotime(date("H:i:s")) > strtotime($row["time_end"]))) {
...
/*function make_time_be_taken($preview_id_to)
{
    // $_POST['day_of_preview']
    $sql_update =  "UPDATE `preview_time_table` SET `is_taken`='1' WHERE id=" . $preview_id_to . "";
    global $pdo;
        ...
        if (isset($_SESSION)  && isset($_SESSION['logged_in'])   && $_SESSION['logged_in'] == "true") {
            ...
            if ($_SESSION['user_type'] == "customer") {
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['day_of_preview']) && !empty($_POST['day_of_preview'])) {
                    ...
                    //make_time_be_taken($_POST['day_of_preview']);
                    //echo "<h2 class='warning'>Your request to preview the flat# ='" . $id . "', is succesfully done,
                    //and has been sent to the owner.<br>Wait for the owner respone!</h2>";
