...
{
    $result_string = "<h2>Some pictures:</h2>  <ul>";
    ...
    while ($row_pic = $result_pic->fetch()) {
        ...
        // if ($i > 0) {
        if ($i % 2 == 0) {
            $result_string .= '<figure class="first_image">'; // this means this is the first img in this row (as flex)
        } else {
...
try {
    ...
    if (!empty($row['id'])) {
        ...
        $first_pic_flex_with_flat_data = '<figure>
        <img src="../images/flat_det_images/' .
            $first_fetch_pic['name'] . '" alt="' . $first_fetch_pic['name'] . '">
            <figcaption>' . $first_fetch_pic['name'] . '</figcaption>
            </figure>';
